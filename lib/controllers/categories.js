...
/**
 * Loop the category table database results and find the parent ID of the requested category
 *
 * @param catId category ID to look for
 * @param data database result object/array
 * @returns {int|boolean}
 * @private
 */
function _findParentCatId(catId, data) {
	var parentId = false;
	for(var i = 0; i < data.length; i++) {
		if(data[i].id === parseInt(catId)) {
...
exports.removeCategory = function(req, res) {
	...
	db.getConnection(function(err, conn) {
		...
		conn.query('SELECT id, parentID from categories', function(err, result) {
			...
			parentCatId	= _findParentCatId(catId, result);
			catIdArr	= _findChildren(catId, result);

			if(catIdArr) {
				catIdArr.join();

				conn.query('insert into categories_backup (id, name, parentID) select id, name, parentID from categories WHERE id IN (' + catIdArr + ')',
					function(err, result) {
						if(err) {
							log.warn('ERR', "DB:QUERY::ERR", err);
							throw err;
						...
						conn.query('DELETE FROM categories WHERE id IN (' + catIdArr + ')',
							function(err, result) {
								if(err) {
									log.warn('ERR', "DB:QUERY::ERR", err);
									throw err;
								...
								conn.query('UPDATE products SET categoryID = ' + parentCatId + ' WHERE categoryID IN (' + catIdArr + ')',
									function(err, result) {
										conn.release();
										if(err) {
											log.warn('ERR', "DB:QUERY::ERR", err);
